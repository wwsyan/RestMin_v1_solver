...
"""
这是一个 Python 代码段，用于导入一些类型提示（Type Hints）所需的类型和泛型，包括：
...
TypeVar：表示泛型变量
Union：表示联合类型
这些类型和泛型通常用于在函数和类中指定参数和返回值的类型，以提高代码的可读性和可维护性。
...
class PolicyGradient(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                policy_loss = -log_prob * rollout_data.returns
