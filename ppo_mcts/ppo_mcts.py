...
class MaskablePPO(OnPolicyAlgorithm):
    ...
    :param: use_mcts: Whether or not to use MCTS for policy improvment
    :param: env_for_mcts: Particular enviroment designed for MCTS 
    """
    ...
    def __init__(
        ...
        if use_mcts:
            try:
                self.mcts = MCTSPlayer(policy_value_function=self.policy, 
                                       env=env_for_mcts, 
                                       device=self.device)
            except:
                raise ValueError("Incorrect MCTSPlayer parameters.")
        else:
            self.mcts = None
        ...
        while n_steps < n_rollout_steps:
            ...
            if self.use_mcts:
                if self.mcts is None:
                    raise ValueError("MCTS object do not exist.")
                current_state = self.env.get_attr("state")[0]
                # 命名上, 之所以是复数形式actions, 是对应多个env的情况
                # 但这里使用的MCTS, 只支持单环境, 不存在多个action. 仍然使用actions是为了保证变量的一致
                actions = np.array([self.mcts.get_action(current_state)])
