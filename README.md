...
## 模式1
由于模式1引入了双色棋子，因此多了异色棋子对换的等效性，与旋转、镜像组合起来一共是16种（[Code]）。
...
如下图所示，同色棋子连成的“块”数是5。如何实现呢？我采用了类似扫雷游戏中展开无雷区的递归算法（[Code]()）。
...
#### 重视调参
在mode0的实验中，我一直使用<code>batch_size=2048</code>，并且效果不错。于是我在mode1的实验中沿用了该参数，但尝试了一些别的参数及微调的奖励组合都效果不好：
